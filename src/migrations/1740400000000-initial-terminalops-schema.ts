...
/** DDL inicial del esquema `terminalops` con IDs numéricos (serial). */
const INITIAL_TERMINALOPS_SCHEMA_SQL = `-- TerminalOps relational schema (PostgreSQL) — normalized (3NF)
...
CREATE TABLE IF NOT EXISTS companies (
  ...
  maintenance_km_control_enabled boolean NOT NULL DEFAULT false,
  maintenance_km_interval_default numeric(10,2),
  maintenance_date_control_enabled boolean NOT NULL DEFAULT false,
  maintenance_date_period_default text
    CHECK (
      maintenance_date_period_default IS NULL
      OR maintenance_date_period_default IN ('monthly', 'quarterly', 'semiannual', 'annual')
    ),
  maintenance_km_control_changed_at timestamptz,
  maintenance_date_control_changed_at timestamptz,
  operational_center_postal_code varchar(5),
  operational_center_city_municipality text,
  operational_center_locality text,
  operational_center_settlement_cons_id varchar(32),
  operational_center_latitude numeric(10,7),
  operational_center_longitude numeric(10,7),
...
CREATE TABLE IF NOT EXISTS fleet_maintenance_entry_documents (
  id serial PRIMARY KEY,
  maintenance_entry_id integer NOT NULL REFERENCES fleet_maintenance_entries(id) ON DELETE CASCADE,
  file_name text NOT NULL,
  sort_order smallint NOT NULL DEFAULT 0
);
...
CREATE TABLE IF NOT EXISTS expenses (
  ...
  related_unit_id integer REFERENCES units(id) ON DELETE SET NULL,
  related_equipment_id integer REFERENCES equipment(id) ON DELETE SET NULL,
  related_operator_id integer REFERENCES operators(id) ON DELETE SET NULL,
...
export class InitialTerminalopsSchema1740400000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(INITIAL_TERMINALOPS_SCHEMA_SQL);
  }
